...
class PygameGraphicsEngine:
    def __init__(self, bounds, width=800, height=600):
        pygame.init()
    ...
    def render(self, player, npcs):
        ...
        pygame.draw.circle(self.screen, (255,0,0), self._map(player.x, player.y), 12)
        for npc in npcs:
            pygame.draw.circle(self.screen, (0,120,255), self._map(npc.x, npc.y), 12)
        pygame.display.flip()
